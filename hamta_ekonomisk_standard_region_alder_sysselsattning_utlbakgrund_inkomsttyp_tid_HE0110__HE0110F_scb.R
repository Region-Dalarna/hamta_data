hamta_ekonomisk_standard_region_alder_sysselsattning_utlbakgrund_inkomsttyp_tid_scb <- function(
    ...
    alder_klartext = "*",			 #  Finns: "20- år", "20-64 år", "65- år", "20-29 år", "30-49 år", "50-64 år", "65-79 år", "80- år"
    sysselsattning_klartext = "*",			 #  NA = tas inte med i uttaget,  Finns: "samtliga personer", "förvärvsarbetande", "icke förvärvsarbetande", "studerande", "arbetslösa", "sjuka", "pensionärer", "föräldralediga", "övriga ej förvärvsarbetande"
    utlbakgrund_klartext = "*",			 #  NA = tas inte med i uttaget,  Finns: "samtliga personer", "utrikes födda", "född i Sverige", "utländsk bakgrund", "inrikes födda med två utrikes födda föräldrar", "svensk bakgrund", "inrikes födda med en inrikes och en utrikes född förälder", "inrikes födda med två inrikes födda föräldrar"
    inkomsttyp_klartext = "*",			 #  Finns: "disponibel inkomst per k.e. inkl. kapitalvinst", "disponibel inkomst per k.e. exkl. kapitalvinst"
    cont_klartext = "*",			 #  Finns: "Medelvärde, tkr", "Medianvärde, tkr", "Medelvärde, procent av samtliga", "Medianvärde, procent av samtliga", "Inkomst < 40 procent", "Inkomst < 50 procent", "Inkomst < 60 procent", "Inkomst < 70 procent", "Inkomst > 200 procent", "Antal personer totalt"
    ...
    excel_filnamn = "ekonomisk_standard.xlsx",			# filnamn för excelfil som exporteras om excel_filnamn och output_mapp anges
  ...
  #
  # Skapad av: frkjon den 07 november 2024
  # Senast uppdaterad: 07 november 2024
  #
  # url till tabellens API: https://api.scb.se/OV0104/v1/doris/sv/ssd/START/HE/HE0110/HE0110F/TabVXDispI69
  #
  # ====================================================================================================
  ...
  url_uttag <- "https://api.scb.se/OV0104/v1/doris/sv/ssd/START/HE/HE0110/HE0110F/TabVXDispI69"
  ...
  sysselsattning_vekt <- if (!all(is.na(sysselsattning_klartext))) hamta_kod_med_klartext(px_meta, sysselsattning_klartext, skickad_fran_variabel = "sysselsattning") else NA
  utlbakgrund_vekt <- if (!all(is.na(utlbakgrund_klartext))) hamta_kod_med_klartext(px_meta, utlbakgrund_klartext, skickad_fran_variabel = "utlbakgrund") else NA
  ...
  varlista <- list(
    "Region" = region_vekt,
    "Alder" = alder_vekt,
    "Sysselsattning" = sysselsattning_vekt,
    "UtlBakgrund" = utlbakgrund_vekt,
    "InkomstTyp" = inkomsttyp_vekt,
    "ContentsCode" = cont_vekt,
    "Tid" = tid_vekt)
  
  if (all(is.na(sysselsattning_klartext))) varlista <- varlista[names(varlista) != "Sysselsattning"]
  if (all(is.na(utlbakgrund_klartext))) varlista <- varlista[names(varlista) != "UtlBakgrund"]
