hamta_integration_transfereringar_region_kon_bakgrund_tid_scb <- function(
			...
			bakgrund_klartext = "*",			 #  Finns: "samtliga 20-64 år", "ålder: 20-24 år", "ålder: 25-34 år", "ålder: 35-44 år", "ålder: 45-54 år", "ålder: 55-64 år", "samtliga utbildningsnivåer", "utbildningsnivå: förgymnasial utbildning", "utbildningsnivå: gymnasial utbildning", "utbildningsnivå: eftergymnasial utbildning", "utbildningsnivå: uppgift saknas", "samtliga", "födelseregion: Sverige", "födelseregion: Norden exkl. Sverige", "födelseregion: EU/EFTA exkl. Norden", "födelseregion: övriga världen", "samtliga utrikes födda invandrare", "skäl till invandring: skyddsbehövande och deras anhöriga", "skäl till invandring: övriga utrikes födda invandrare", "samtliga utrikes födda", "vistelsetid 0-1 år", "vistelsetid 2-3 år", "vistelsetid 4-9 år", "vistelsetid 10- år", "män och kvinnor", "män", "kvinnor"
			cont_klartext = "*",			 #  Finns: "Andel sjukpenning, sjuk-och aktivitetsersättning m.m, av nettoinkomsten, procent", "Andel sjukpenning av nettoinkomsten, procent", "Andel sjuk- och aktivitetsersättning av nettoinkomsten, procent", "Andel arbetslöshetsersättning av nettoinkomsten, procent", "Andel försörjningsstöd av nettoinkomsten, procent", "Andel introduktionsersättning av nettoinkomsten, procent", "Andel etableringsersättning av nettoinkomsten, procent", "Andel arbetslöshetsersättning av nettoinkomsten , procent"
			tid_koder = "*",			 # "*" = alla år eller månader, "9999" = senaste, finns: "2002", "2003", "2004", "2005", "2006", "2007", "2008", "2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018", "2019", "2020", "2021", "2022", "2023"
			long_format = TRUE,			# TRUE = konvertera innehållsvariablerna i datasetet till long-format 
			wide_om_en_contvar = TRUE,			# TRUE = om man vill behålla wide-format om det bara finns en innehållsvariabel, FALSE om man vill konvertera till long-format även om det bara finns en innehållsvariabel
			output_mapp = NA,			# anges om man vill exportera en excelfil med uttaget, den mapp man vill spara excelfilen till
			excel_filnamn = "integration_transfereringar.xlsx",			# filnamn för excelfil som exporteras om excel_filnamn och output_mapp anges
  ...
  #
  # Skapad av: frkjon den 02 april 2025
  # Senast uppdaterad: 02 april 2025
  #
  # url till tabellens API: https://www.statistikdatabasen.scb.se/pxweb/sv/ssd/START__AA__AA0003__AA0003G/IntGr4RikKON/
  #												https://www.statistikdatabasen.scb.se/pxweb/sv/ssd/START__AA__AA0003__AA0003G/IntGr4LanKON/
  #												https://www.statistikdatabasen.scb.se/pxweb/sv/ssd/START__AA__AA0003__AA0003G/IntGr4Kom/
  #
  # ====================================================================================================
  ...
  # Url till databas
  url_list <- c("https://api.scb.se/OV0104/v1/doris/sv/ssd/START/AA/AA0003/AA0003G/IntGr4RikKON",
						"https://api.scb.se/OV0104/v1/doris/sv/ssd/START/AA/AA0003/AA0003G/IntGr4LanKON",
						"https://api.scb.se/OV0104/v1/doris/sv/ssd/START/AA/AA0003/AA0003G/IntGr4Kom")
 ...
 hamta_data <- function(url_uttag) {
  ...
  tid_vekt <- if (all(tid_koder != "*")) tid_koder %>% as.character() %>% str_replace("9999", max(giltiga_ar)) %>% .[. %in% giltiga_ar] %>% unique() else giltiga_ar
	...
	if (length(region_giltig_vekt) > 0) {
	  ...
	  if (long_format & !wide_om_en_contvar) px_df <- px_df %>% konvertera_till_long_for_contentscode_variabler(url_uttag, content_var = "transferering")
